...
class Easy:
    def start_game(self, count):
        ...
        if count > 0:
            ...
            self.last_score.config(text=f"Ваш счёт {score}.Хорошая работа!!")
            self.text_entry.config(state=DISABLED)
        elif count == 0 and 20 < score < 30:
            self.last_score.config(text=f"Ваш счёт  {score}.Хорошо!")
            self.text_entry.config(state=DISABLED)
        elif count == 0 and 10 < score < 20:
            self.last_score.config(text=f"Ваш счёт{score}. Средний :)")
            self.text_entry.config(state=DISABLED)
        elif count == 0 and score == 0 or score < 10:
            self.last_score.config(text=f"Ваш счёт {score}.Попробуйте снова!")
            self.text_entry.config(state=DISABLED)
