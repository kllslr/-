...
class Medium:
    def start_game(self, count):
        ...
        if count > 0:
            ...
            self.last_score.config(text=f"Ваш счёт  {med_score}.Отличная работа!!")
            self.text_entry.config(state=DISABLED)
        elif count == 0 and 5 < med_score < 10:
            self.last_score.config(text=f"Ваш счёт {med_score}.хорошо!")
            self.text_entry.config(state=DISABLED)
        elif count == 0 and 0 < med_score < 5:
            self.last_score.config(text=f"Ваш счёт {med_score}.Средний :)")
            self.text_entry.config(state=DISABLED)
        elif count == 0 and med_score == 0:
            self.last_score.config(text=f"Ваш счёт {med_score}.Попробуйте снова!")
            self.text_entry.config(state=DISABLED)
    ...
    def __init__(self):
        ...
        self.top_window2.resizable(width=False, height=False)
        self.top_window2.title("Уровень Средний")
        ...
        self.message = Label(self.top_window2, text="Нажмите кнопку Start, чтобы начать.Введите слово и нажмите Enter."
                                                    "У вас будет 60 секунд", font=('Courier', 10, 'bold'),
