...
class Hard:
    def start_game(self, count):
        ...
        if count > 0:
            ...
            self.last_score.config(text=f"Ваш счёт {hard_score}Хорошая работа!!")
            self.text_entry.config(state=DISABLED)
        elif count == 0 and 0 < hard_score < 5:
            self.last_score.config(text=f"Ваш счёт {hard_score}.Хорошо!")
            self.text_entry.config(state=DISABLED)
        elif count == 0 and hard_score == 0:
            self.last_score.config(text=f"Ваш счёт {hard_score}.Попробуйте снова :(")
            self.text_entry.config(state=DISABLED)
    ...
    def __init__(self):
        ...
        self.top_window3.resizable(width=False, height=False)
        self.top_window3.title("Уровень Сложный")
        ...
        self.message = Label(self.top_window3, text="Нажмите кнопку Start, чтобы начать.Введите слово и нажмите Enter."
                                                    "У вас будет 30 секунд", font=('Courier', 10, 'bold'),
